...
export default {
  ...
  methods: {
    ...
    ImgDocumentArr(arr) {
      const vm = this
      this.$http.httpRequestImgConfig(vm.orderDetailData[vm.flgNumber].type).then(response => {
        // 修改参数
        if (response.response.status === 200 || response.status === 200) {
          let sortId
          if (vm.modification) {
    ...
    SubmitParams(params) {
      ...
      this.AddProductloading = true
      if (!vm.listImg.url) {
        this.$http.httpRequestImgConfig(vm.listImg.type).then(response => {
          if (response.response.status === 200 || response.status === 200) {
            // 修改参数
    ...
    thumbnailFile(vm, vueObj, params) {
      if (!vm.thumbnail.url) {
        vm.$http.httpRequestImgConfig(vm.thumbnail.type).then(response => {
          if (response.response.status === 200 || response.status === 200) {
            // 修改参数
